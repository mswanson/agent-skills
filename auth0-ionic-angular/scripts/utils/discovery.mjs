...
export async function discoverExistingConnections() {
  ...
  try {
    ...
    spinner.succeed(`Discovered ${connections.length} existing connection(s)`)
    return connections
  } catch (e) {
...
export function buildChangePlan(connections, domain, ionicConfig) {
  const clientPlan = checkNativeClientChanges(domain, ionicConfig)
